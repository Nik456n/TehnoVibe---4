...
# ============================================
# РЕЗУЛЬТАТЫ РАЗБОРА ДЛЯ MINI APP
# ============================================
# Бот считает анализ у себя и кладёт результат сюда под ключом чата.
# Кнопка «Открыть отчёт» ведёт на Mini App с ?rid=<ключ>, фронт
# забирает готовый разбор и показывает выписку пользователя, а не демо.
#
# Хранится в памяти процесса: перезапуск бэкенда очищает. Для демо
# этого достаточно, база тут была бы лишней сущностью.
# Ключ знает только тот, кому бот прислал кнопку, — но это не защита,
# а демо. Персональных данных в ответе нет: только названия сервисов.
RESULTS: dict[str, dict] = {}


@app.post("/results/{key}")
async def save_result(key: str, payload: AnalyzeResponse):
    """Бот кладёт сюда готовый разбор. Схема проверяется на входе."""
    RESULTS[key] = payload.model_dump(mode="json")
    return {"stored": key, "subscriptions": len(payload.subscriptions)}


@app.get("/results/{key}", response_model=AnalyzeResponse)
async def get_result(key: str):
    """Mini App забирает разбор по ключу из ?rid=."""
