...
def _parse_amount(raw: str) -> float | None:
    ...
    try:
        ...
        return None
    # Нас интересуют только списания. Явный плюс — это поступление
    # (зарплата, аванс, перевод): такие операции пропускаем совсем.
    # Без этого зарплата в 65 000 ₽, приходящая шестой месяц подряд с
    # ровным интервалом, выглядит для детектора идеальной подпиской.
    # Если знака нет — считаем расходом: так устроены CSV-выписки.
