...
def detect(transactions: list[dict],
    ...
    #
    # Категория "other" из поиска исключена намеренно. Туда попадает всё,
    # что не опознано справочником, и это свалка, а не функциональная
    # группа: два неизвестных платежа не заменяют друг друга просто
    # потому, что оба неизвестны. Без этого фильтра коммунальный платёж
    # с фиксированной суммой объявлялся дублем Apple Services, и в план
    # уходила рекомендация отключить квартплату.
    #
    # Такие подписки остаются в списке — они действительно регулярные, —
    # но без флага "лишняя" и без совета отменить.
    NON_FUNCTIONAL = {"other"}

    by_category: dict[str, list[dict]] = defaultdict(list)
    for s in subscriptions:
        if s["category"] in NON_FUNCTIONAL:
            continue
