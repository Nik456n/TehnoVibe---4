...
def run_pipeline(content: bytes, filename: str) -> dict:
    """Здесь будет парсер + детектор (пишет тимлид).

    Пока возвращает мок. Когда детектор будет готов, меняется только
    тело этой функции — контракт и эндпоинты остаются прежними.
    """
    return MOCK
...
async def cancel(subscription_id: str):
    sub = next((s for s in MOCK["subscriptions"] if s["id"] == subscription_id), None)
    if sub is None:
        raise HTTPException(
...
async def analyze_with_ai(file: UploadFile = File(...)):
    ...
    if not GIGACHAT_API_KEY:
        raise HTTPException(status_code=503, detail="LLM_API_KEY не задан в .env")
